...
class BluetoothKalmanFilter {
  ...
  double exponentialWeight(double rssi) {
    return pow(10, -0.1 * rssi.abs()).toDouble();
  }

  MapEntry<String?, Map<String, Map<String, dynamic>>> getNearestBeacon(
      Map<String, List<double>> beacons,
      {double minStrength = 95.0, double minDelta = 1.5, int minReadings = 1}) {
    ...
    beacons.forEach((id, rssiList) {
      ...
      if (cleaned.length < minReadings) return;
